 ...
 */
export function buildCvHtml({ form, experiences, education, projects, courses, certifications, techSkills, softSkills, splitLines, splitList, lang = "ar" }) {
    ...
    sections.push(`
      <section>
        ...
        ${certs.map((x) => `
          <div class="entry">
            <div class="entry-head">
              <span class="entry-title">${esc(x.name)}${x.issuingBody ? ` — ${esc(x.issuingBody)}` : ""}</span>
              ${x.date ? `<span class="entry-meta">${esc(x.date)}</span>` : ""}
            </div>
          </div>`).join("")}
      </section>`);
  }
