...
// LIGATURE-SCRAMBLING FIX — Chromium/PDFium mis-maps certain mandatory
// Arabic ligature glyphs in the PDF's text layer (ToUnicode CMap), so a
// word containing one comes out with its character order scrambled when
// copy/pasted or run through pdftotext/pdf-parse, even though on-screen
// and printed rendering is correct either way. Confirmed via isolated
// repro against two independent extractors (pdftotext, pdf-parse) for two
// cases: the mandatory Lam-Alef ligature ("لا", incl. at the very common
// "ال" article + alef-initial-word boundary, e.g. "الاقتصاد") and the
// dedicated "Allah" ligature (U+FDF2) that most Arabic fonts substitute
// for the literal sequence "الله" (e.g. "عبدالله" -> corrupted "عبدهللا"/
// "ﷲعبد" depending on the extractor).
//
...
//
// The actual fix: disable ligature substitution outright via CSS (see
// font-variant-ligatures/font-feature-settings on `body` below) so the
// ligature glyph never forms — Lam and Alef (or the "الله" sequence)
// render as their normal, separately-shaped, still cursively-JOINED
// letterforms instead of the special combined glyph. Verified visually
// (letters stay connected, unlike the reverted ZWNJ attempt — see
// debug/generate-test-cv.mjs) and via both extractors: both cases now
// come out with the correct, unscrambled character sequence.
//
// STILL UNRESOLVED: a word containing a shadda (gemination mark, U+0651,
// e.g. "متمكّن") scrambles independently of ligatures — confirmed the
// ligature-substitution fix above does NOT touch it (identical corruption
// with or without it). This is a mark-positioning issue, a different
// Chromium/PDFium mechanism from ligature substitution, and remains an
// unavoidable bug for now.
function esc(str) {
...
// Same helper duplicated in components/AtsCvBuilder.jsx (the client-side
// form) — computes the rendered GPA line from the RAW gpaValue/gpaScale
// rather than trusting the client's own already-formatted string, so a
// request that bypasses the client's own gpaExceedsScale check (see
// generate-pdf's route-level validation, which rejects the request
// outright) can never reach here with an actually-invalid pair in the
// first place; this is the second, redundant line of defense that
// renders nothing at all if it somehow did.
 ...
 */
export function buildCvHtml({ form, experiences, education, courses, certifications, customSections, techSkills, softSkills, skillDetails, splitLines, splitList, lang = "ar" }) {
  ...
  // now — for every CV, not just a sparse one — so whenever at least one
  // is present the group renders as a bulleted "**Name**: description"
  // list, name and description inline on one line. Professional/soft
  // skills NEVER have descriptions, for anyone — rendered as a 2-column
  // bulleted grid of plain names instead of the description list.
  const techDetails = (skillDetails?.tech || []).filter((s) => s.name);
  function skillDetailBullets(details) {
    return `<ul class="bullets">${details.map((s) => `<li><strong>${esc(s.name)}</strong>${s.description ? `: ${esc(s.description)}` : ""}</li>`).join("")}</ul>`;
  }
  // CSS multi-column layout fills the first column top-to-bottom before
  // moving to the next — and, under dir="rtl" (set on <html> above), the
  // first column renders on the right, matching the requested "top-right,
  // flowing right-to-left then down" order without any JS reflow logic.
  function skillsGrid(names) {
    return `<ul class="bullets skills-grid">${names.map((s) => `<li>${esc(s)}</li>`).join("")}</ul>`;
  }
  ...
  if (techSkills || softSkills) {
    sections.push(`
      <section>
        <h2>${esc(t.skills)}</h2>
        ${techHasDescriptions
          ? `<p><strong>${esc(t.techSkills)}</strong></p>${skillDetailBullets(techDetails)}`
          : (techSkills ? `<p><strong>${esc(t.techSkills)}</strong> ${esc(splitList(techSkills).join(" | "))}</p>` : "")}
        ${softSkills ? `<p><strong>${esc(t.softSkills)}</strong></p>${skillsGrid(splitList(softSkills))}` : ""}
      </section>`);
  }
        ...
        ${edus.map((x) => `
          <div class="entry">
            ...
            ${(() => {
              ...
              return gpaLine ? `<div class="entry-detail"><strong>${esc(t.gpa)}:</strong> ${esc(gpaLine)}</div>` : "";
            })()}
  ...
  // User-defined sections, in the order the applicant added them.
  customs.forEach((x) => {
    sections.push(`
      <section>
  ...
  // Languages is always the absolute last section, after any custom
  // sections — see the matching order in AtsCvBuilder.jsx's preview JSX.
  if (form.languages) {
    sections.push(`
      <section>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  body {
    /* Disables mandatory ligature substitution (Lam-Alef, the "الله"
       ligature) — see the LIGATURE-SCRAMBLING FIX comment at the top of
       this file for why: those glyphs' PDF text-layer mapping comes out
       scrambled, and this avoids the glyph forming at all. Does NOT
       affect normal Arabic cursive joining, which is a separate, always-
       on shaping stage untouched by these properties. */
  ...
  /* Professional skills: 2-column bulleted grid instead of a single
     joined line — column-count naturally respects the page's dir (RTL
     starts the first column on the right), and break-inside keeps a
     single skill from splitting across the column break. */
  ul.skills-grid { column-count: 2; column-gap: 22px; }
  ul.skills-grid li { break-inside: avoid; }
