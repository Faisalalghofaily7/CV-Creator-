        ...
        ${crs.map((x) => `
          <div class="entry">
            <div class="entry-head">
              ...
              ${x.hours ? `<span class="entry-meta">${esc(x.hours)} ${esc(t.hoursUnit)}</span>` : ""}
            </div>
          </div>`).join("")}
      </section>`);
  }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .objective { font-size: 11.5px; color: #333333; margin-top: 3px; font-style: italic; }
...
<body>
  <div class="cv-page">
    <div class="header">
      ...
      ${form.objective ? `<div class="objective">${esc(t.objective)}: ${esc(form.objective)}</div>` : ""}
