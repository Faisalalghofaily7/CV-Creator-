...
export function buildCvHtml({ form, experiences, education, courses, certifications, customSections, techSkills, softSkills, skillDetails, splitLines, splitList, lang = "ar" }) {
  ...
  // skillDetails carries a per-skill description ONLY for a sparse CV (no
  // experience — see rule #7/isSparseCv); every normal CV renders the exact
  // same plain joined-list line it always has. When present, each group
  // renders as a bulleted "Name — description" list instead.
  const techDetails = (skillDetails?.tech || []).filter((s) => s.name);
  const softDetails = (skillDetails?.soft || []).filter((s) => s.name);
  ...
  if (techSkills || softSkills || techDetails.length || softDetails.length) {
    sections.push(`
      <section>
        <h2>${esc(t.skills)}</h2>
        ${techDetails.length
        ...
        ${softDetails.length
          ? `<p><strong>${esc(t.softSkills)}</strong></p>${skillDetailBullets(softDetails)}`
