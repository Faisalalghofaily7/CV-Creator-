...
export async function createAccessCode({
  ...
  // linkedin_status is deliberately never set here — it stays NULL at
  // creation for every package. LinkedIn-only orders never reach this
  // function (see lib/linkedinOrdersDb.js); an Integrated code's LinkedIn
  // track only starts once the CV is actually generated (lib/cvArchive.js),
  // since Staff2's work depends on the CV existing.
  const sql = getSql();
  for (let attempt = 0; attempt < 5; attempt++) {
    const code = generateCode();
    try {
      const [row] = await sql`
        INSERT INTO access_codes (code, salla_order_number, status, applicant_name, applicant_email, applicant_phone, salla_order_status, requested_package, generation_source, created_by)
        VALUES (${code}, ${sallaOrderNumber}, 'available', ${applicantName}, ${applicantEmail}, ${applicantPhone}, ${sallaOrderStatus}, ${requestedPackage}, ${generationSource}, ${createdBy})
