  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      const [row] = await sql`
        ...
        RETURNING id, code, salla_order_number, status, lifecycle_status, applicant_name, applicant_phone, requested_package, generation_source, created_by, created_at, used_at
      `;
      return row;
    } catch (err) {
