 ...
 */
export async function createAccessCode({ sallaOrderNumber = null, applicantName = null, applicantEmail = null, applicantPhone = null } = {}) {
  const sql = getSql();
  for (let attempt = 0; attempt < 5; attempt++) {
    const code = generateCode();
    try {
      const [row] = await sql`
        INSERT INTO access_codes (code, salla_order_number, status, applicant_name, applicant_email, applicant_phone)
        VALUES (${code}, ${sallaOrderNumber}, 'available', ${applicantName}, ${applicantEmail}, ${applicantPhone})
