...
export async function createAccessCode({
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      const [row] = await sql`
        INSERT INTO access_codes (code, salla_order_number, status, applicant_name, applicant_email, applicant_phone, salla_order_status, requested_package)
        VALUES (${code}, ${sallaOrderNumber}, 'available', ${applicantName}, ${applicantEmail}, ${applicantPhone}, ${sallaOrderStatus}, ${requestedPackage})
        RETURNING id, code, salla_order_number, status, applicant_name, applicant_phone, requested_package, created_at, used_at
      `;
      return row;
    } catch (err) {
