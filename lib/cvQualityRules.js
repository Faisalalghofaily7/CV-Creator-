...
// for a sparse CV, every one of the required 5 must actually have one.
// Professional/soft skills never have descriptions at all (plain tags,
// for every user), so the sparse minimum there is a bare count.
function sparseTechOk(tags, descriptions) {
 ...
 * "sparseSkillsMinimum".
 */
export function getCvQualityIssues({ name, experiences, courses, techSkillTags, softSkillTags, techSkillDescriptions }) {
  const issues = [];
  ...
  if (isSparseCv(experiences)) {
    if (!sparseTechOk(techSkillTags, techSkillDescriptions) || !sparseSoftOk(softSkillTags)) issues.push("sparseSkillsMinimum");
  }
