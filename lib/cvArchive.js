...
export async function archiveGeneratedPdf({ accessCode, pdfBuffer, applicantName, applicantEmail, applicantPhone, applicantCity, applicantTargetRole, lang }) {
  ...
  try {
    ...
    // First time this code is archived (not a re-generation) — record the
    // starting point of its sending-status timeline. The column itself
    // already defaults to 'pending', this just makes that moment visible
    // in the history shown to staff.
    if (!previousPathname) {
      await sql`INSERT INTO sending_status_history (access_code_id, status, changed_by) VALUES (${updated.id}, 'pending', NULL)`;
    }
