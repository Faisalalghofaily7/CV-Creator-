...
import { notifyLinkedinStaff } from "./linkedinOrdersDb";
  ...
  try {
    ...
    if (!previousPathname) {
      ...
      if (packageNotifiesLinkedin(requestedPackage)) {
        ...
        await sql`
          UPDATE access_codes
          SET linkedin_notification_status = ${status}, linkedin_notified_email = ${emails.join(", ") || null}, linkedin_notified_at = now()
          WHERE id = ${updated.id}
        `;
      }
    }
