...
// Narrower than staffCanAccessRecord above, and used ONLY for viewing/
// downloading the archived CV PDF itself (read-only — no status change,
// no history, no listing). A 'linkedin' staff member additionally gets
// this for an Integrated-package record: they see that order in their own
// LinkedIn queue and need to open the CV that came with it, but this does
// NOT grant them the rest of the CV-archive routes (list/history/status),
// which stay exactly as staffCanAccessRecord already gates them.
export function staffCanViewCvPdf(staffType, requestedPackage) {
  if (staffCanAccessRecord(staffType, requestedPackage)) return true;
  return staffType === "linkedin" && requestedPackage === PACKAGE_INTEGRATED;
}
