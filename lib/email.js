...
function buildRenewalNotificationHtml({ applicantName, code, sallaOrderNumber }) {
  const adminUrl = `${APP_URL}/admin`;
  const rows = [
    ["اسم المتقدم", applicantName],
    ["الكود", code],
  ...
  return `
    <div dir="rtl" style="font-family: 'Segoe UI', Tahoma, sans-serif; color: #3a4a5a; max-width: 480px; margin: 0 auto; line-height: 1.8;">
      <p>مرحباً،</p>
      <p style="font-weight: 700; color: #b3261e;">⚠️ تم تجديد كود سبق استخدامه — العميل سيُعِدّ سيرة ذاتية جديدة من جديد.</p>
      <p style="font-weight: 700;">لا ترسل السيرة الذاتية القديمة لهذا الطلب — بانتظار سيرة جديدة.</p>
      <ul style="list-style: none; padding: 0; margin: 16px 0;">${rows}</ul>
      <p style="margin-top: 24px;">
        <a href="${adminUrl}" style="display: inline-block; background: #1a3a5c; color: #ffffff; text-decoration: none; padding: 10px 22px; border-radius: 8px; font-weight: 700; font-size: 14px;">
          الدخول إلى لوحة التحكم
...
/**
 * Notifies one sending/LinkedIn-staff recipient by email that an admin
 * renewed a code that had already been used — see lib/adminRenewal.js for
 * the full renewal flow. Same throw/catch contract as the other
 * notification functions in this file: the caller (lib/adminRenewal.js)
 * catches this per-recipient so one bad address never blocks the others
 * or the renewal itself.
 */
export async function sendStaffRenewalNotification({ to, applicantName, code, sallaOrderNumber }) {
  if (!to) {
    console.warn("Skipping renewal notification email: no recipient given.");
    return;
  }
  ...
  const result = await client.emails.send({
    from: FROM_ADDRESS,
    to,
    subject: `⚠️ تم تجديد كود — بانتظار سيرة جديدة${sallaOrderNumber ? ` — طلب #${sallaOrderNumber}` : ""}`,
    html: buildRenewalNotificationHtml({ applicantName, code, sallaOrderNumber }),
  });
