...
/**
 * Draws `text` anchored at `x` (right edge if align:"right", left edge if
 * align:"left") with correctly joined, bidi-laid-out text for human
 * reading, PLUS an invisible duplicate (plain, unshaped, left-to-right
 * glyph order matching the original logical string) at the same spot — an
 * OCR-style invisible text layer. Extractors reconstruct text from glyph
 * positions; the invisible layer's glyphs are laid out in simple
 * ascending-x / logical order, so any position-based extractor (virtually
 * all of them, including ATS parsers) recovers the exact original string,
 * while a human only ever sees the shaped visible layer on top.
 *
 * The bidi-reordered `visual` string is always laid out left-to-right —
 * that's what bidi reordering produces regardless of whether the text is
 * RTL- or LTR-dominant — so the same drawing logic anchors correctly from
 * either edge; only the starting x differs.
 */
export function bidiText(doc, text, x, y, { align = "right", color } = {}) {
  if (!text) return;
  ...
  drawGlyphRun(doc, visual, visualUnicode, startX, y, { color });

  const identityUnicode = (ch) => ch.charCodeAt(0);
  drawGlyphRun(doc, text, identityUnicode, startX, y, { color, invisible: true });
}

/** Right-anchored shorthand for {@link bidiText}. */
export function rtlText(doc, text, rightX, y, opts) {
  return bidiText(doc, text, rightX, y, { ...opts, align: "right" });
}

/** Left-anchored shorthand for {@link bidiText}. */
export function leftAlignedText(doc, text, leftX, y, opts) {
  return bidiText(doc, text, leftX, y, { ...opts, align: "left" });
}

/** Word-wraps `text` to `maxWidth` (current font/size) and draws each line anchored at `x`, returning the new cursor Y. */
export function wrappedText(doc, text, x, y, { maxWidth, lineHeight, align = "right", color } = {}) {
  if (!text) return y;
  const lines = doc.splitTextToSize(text, maxWidth);
  lines.forEach((line, i) => bidiText(doc, line, x, y + i * lineHeight, { align, color }));
...
export function rtlParagraph(doc, text, rightX, y, maxWidth, lineHeight, opts) {
  return wrappedText(doc, text, rightX, y, { maxWidth, lineHeight, align: "right", ...opts });
}
