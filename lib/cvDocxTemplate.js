 ...
 */
export function buildCvDocx({ form, experiences, education, courses, certifications, customSections, techSkills, softSkills, lang = "ar" }) {
  const isAr = lang !== "en";
  ...
  if (techSkills || softSkills) {
    ...
    if (techSkills) children.push(para([run(t.techSkills, { bold: true, size: 21 }), run(` ${splitList(techSkills).join(" | ")}`, { size: 21 })], { spacing: { after: 40 } }));
    if (softSkills) children.push(para([run(t.softSkills, { bold: true, size: 21 }), run(` ${splitList(softSkills).join(" | ")}`, { size: 21 })], { spacing: { after: 40 } }));
  }
