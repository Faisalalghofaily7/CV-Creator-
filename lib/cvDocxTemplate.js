...
export function buildCvDocx({ form, experiences, education, courses, certifications, customSections, techSkills, softSkills, skillDetails, lang = "ar" }) {
  ...
  if (techSkills || softSkills || techDetails.length || softDetails.length) {
    ...
    if (softDetails.length) {
      children.push(para([run(t.softSkills, { bold: true, size: 21 })], { spacing: { after: 20 } }));
      softDetails.forEach((s) => children.push(bulletPara(s.description ? `${s.name} — ${s.description}` : s.name)));
    } else if (softSkills) {
      children.push(para([run(t.softSkills, { bold: true, size: 21 }), run(` ${splitList(softSkills).join(" | ")}`, { size: 21 })], { spacing: { after: 40 } }));
    }
  }
