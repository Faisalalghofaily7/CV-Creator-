...
### حذف طلب نهائيًا وتجديد كود مُستخدَم (للمشرف فقط)

يستخدم كلا الإجراءين أعمدة/جداول جديدة (`renewed_at`/`renewed_by` على `access_codes`، وجدولا `renewal_log`/`deletion_log`) — شغّل `node --env-file=.env.development.local scripts/init-db.mjs` (أو نفّذ `scripts/schema.sql` من محرر SQL في Neon كما في قسم "إنشاء/تحديث الجدول" أعلاه) بعد النشر مرة واحدة قبل استخدامهما.

**الحذف النهائي** — زر "حذف" (بجانب كل طلب في تبويب "الطلبات"، وبطاقات "أرشيف السير الذاتية"، وتبويب "طلبات لينكدإن") يطلب تأكيدًا صريحًا، ثم يحذف السجل نهائيًا مع كل سجلاته التاريخية المرتبطة (عبر `ON DELETE CASCADE`، تمامًا كما في تنظيف قاعدة البيانات اليدوي) وملف الـ PDF المؤرشف في Blob إن وُجد. متاح للمشرف فقط، ومفروض على مستوى الخادم (`requireRole: "admin"` في `DELETE /api/admin/codes/[id]` و`DELETE /api/admin/linkedin-orders/[id]`) — لا يظهر الزر للموظف أصلًا، وحتى لو استُدعيت نقطة النهاية مباشرة تُرفض بخطأ 403. كل عملية حذف تُسجَّل في جدول `deletion_log` (من حذف، ومتى، وهوية السجل المحذوف) لأن السجل نفسه يختفي بعدها.

**تجديد كود مُستخدَم** — زر "تجديد" (يظهر فقط على الأكواد التي استُخدمت فعلًا) يعيد الكود إلى حالة "متاح" للاستخدام من جديد، **دون حذف الـ PDF الحالي** — يبقى قابلاً للعرض والتحميل حتى يُصدِّر العميل سيرة جديدة فعليًا، فلا تنشأ فجوة بلا سيرة ذاتية لهذا الطلب. تظهر شارة/بانر تحذيري "تم تجديد الكود — بانتظار سيرة جديدة، لا ترسل السيرة القديمة" في تبويب "الطلبات" وبطاقة الأرشيف وبطاقة لينكدإن (إن كانت الباقة متكاملة) حتى يُصدَّر ملف جديد فعليًا، ويصل بريد تحذيري بنفس الرسالة لموظفي الإرسال (ولموظفي لينكدإن أيضًا إن كان مسار لينكدإن متأثرًا) عبر Resend. بمجرد تصدير سيرة جديدة، يستبدل الملف الجديد القديم تلقائيًا (نفس آلية إعادة التصدير الحالية) ويختفي البانر ويستأنف الطلب مساره الطبيعي عبر الحالات المعتادة — والكود يعود خاضعًا لقاعدة الاستخدام الواحد من جديد. متاح للمشرف فقط ومفروض على مستوى الخادم (`POST /api/admin/codes/[id]/renew`)، وكل تجديد يُسجَّل في جدول `renewal_log` (من جدّد، ومتى، لأي كود) لأن هذا الإجراء يتجاوز عمدًا حماية الاستخدام الواحد.
