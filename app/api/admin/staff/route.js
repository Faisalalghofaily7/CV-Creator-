...
export async function GET(request) {
  ...
  try {
    ...
    const rows = await sql`
      SELECT id, username, display_name, staff_type, email, active, created_at
      FROM staff_accounts
...
export async function POST(request) {
  ...
  try {
    ...
    // Optional at creation (the admin can add it later) — but if given, it
    // must actually be a valid address, since it's about to become a real
    ...
    const sql = getSql();
    const [row] = await sql`
      INSERT INTO staff_accounts (username, password_hash, display_name, staff_type, email)
      VALUES (${username}, ${passwordHash}, ${displayName}, ${staffType}, ${email || null})
